...
results_dir = r'C:\Users\jesal\Downloads\archive'
...
for letter in os.listdir(dataset_dir):
    ...
    for file in os.listdir(letter_dir):
        ...
        for angle in rotation_angles:
            ...
            rotated_annotated = cv2.warpAffine(annotated_image, rot_matrix, (w, h))
            rotated_skeleton = cv2.warpAffine(blank_image, rot_matrix, (w, h))
            base_name, ext = os.path.splitext(file)
            rotated_file_name = f"{base_name}_rot{angle}{ext}"
            out_rot_path_with_bg = os.path.join(out_dir_with_bg, rotated_file_name)
            out_rot_path_skel = os.path.join(out_dir_skel, rotated_file_name)
            cv2.imwrite(out_rot_path_with_bg, rotated_annotated)
            cv2.imwrite(out_rot_path_skel, rotated_skeleton)

        translations = [(10, 0), (-10, 0), (0, 10), (0, -10)]
        for tx, ty in translations:
            trans_matrix = np.float32([[1, 0, tx], [0, 1, ty]])
            translated_annotated = cv2.warpAffine(annotated_image, trans_matrix, (w, h))
            translated_skeleton = cv2.warpAffine(blank_image, trans_matrix, (w, h))
            base_name, ext = os.path.splitext(file)
            trans_file_name = f"{base_name}_trans{tx}_{ty}{ext}"
            out_trans_path_with_bg = os.path.join(out_dir_with_bg, trans_file_name)
            out_trans_path_skel = os.path.join(out_dir_skel, trans_file_name)
            cv2.imwrite(out_trans_path_with_bg, translated_annotated)
            cv2.imwrite(out_trans_path_skel, translated_skeleton)
