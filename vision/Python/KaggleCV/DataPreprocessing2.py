...
FINAL_W, FINAL_H = 200, 200  # Final image dimensions
TARGET_COUNT = 3000  # Target number of images per folder
...
# Mediapipe hands setup and hand connections (as in your example)
...
# Define helper augmentation functions
...
# List of augmentation methods: each returns a transformed image.
aug_methods = [
...
# First pass: Process each input image to create a base image.
with mp_hands.Hands(static_image_mode=True, max_num_hands=1, min_detection_confidence=0.5) as hands:
    for folder_name in os.listdir(input_root):
        ...
        for image_name in os.listdir(subfolder_input):
            ...
            # Determine scaling factor with a 0.9 margin
            ...
            # Transform points: scale and center them
            transformed_points = []
            for x, y in pts:
            ...
            # Create a blank image and draw the skeleton
            ...
            # Optionally flip if hand is reported as "Right"
            ...
            # Save the base image with a '_base' suffix.
...
# Augmentation pass: For each folder, if count < TARGET_COUNT, augment until reaching 3000 images.
for folder_name, base_count in folder_counts.items():
    ...
    aug_index = 0  # To generate unique augmentation filenames
    ...
    while current_count < TARGET_COUNT:
        ...
        # Randomly select an augmentation method from the list
        aug_name, aug_func = random.choice(aug_methods)
        aug_img = aug_func(base_img)

        # Create a new filename using the base name and an augmentation index
...
# Final summary of image counts per folder
