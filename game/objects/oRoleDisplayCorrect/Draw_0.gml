if (global.oRoleGameSprites != undefined) {
    var count = ds_list_size(global.oRoleGameSprites);
    if (count > 0) {
        var scale = 0.75;
        var gap = 20;
        ...
        for (var i = 0; i < count; i++) {
            var spr = global.oRoleGameSprites[| i][? "sprite"];
        ...
        for (var i = 0; i < min(visible_sprites, count); i++) {
            var spr_data = global.oRoleGameSprites[| i];
